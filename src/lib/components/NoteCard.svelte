...
<a
  ...
  class={twMerge("flex flex-col p-4 gap-2 rounded transition-all border border-gray ring-2 bg-black relative group", noteBorderColors.find(el => el.name === note.color)?.class ?? 'ring-0', className)}
>
  <h2>{note.title}</h2>
  {#if note.type === 'list'}
    <ul>

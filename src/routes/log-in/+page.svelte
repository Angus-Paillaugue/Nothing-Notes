...
<section class="flex flex-col items-center justify-center h-screen p-2">
    <div class="p-4 rounded flex flex-col gap-4 bg-gray min-w-[500px]">
        <h1>Log In</h1>
        <form class="flex flex-col gap-2" method="POST" use:enhance>
            ...
            <button type="submit" class="w-full px-4 py-2 text-lg bg-black rounded mt-2">Log In</button>
        </form>
        {#if form?.error}
            <div class="p-6 rounded-full bg-black flex flex-row gap-4 items-center">
                <div class="rounded-full w-16 h-16 bg-red flex flex-col items-center justify-center">
        ...
        {/if}
        <a href="/sign-up" class="text-sm">Don't have an account? Sign up</a>
    </div>
</section>
